...
const ManageAnnouncements = () => {
  ...
  const { data: items, isLoading, error, refetch } = useListData(() => masjidDB.get('announcements'));

  const handleDelete = async (id: string) => {
    if (window.confirm('Are you sure you want to delete this record?')) {
      try {
        await masjidDB.delete('announcements', id);
    ...
    <div>
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">View and manage announcements</p>
        </div>
        <Link to="/masjid/admin/announcements/new">
      ...
      <DataState
        ...
        emptyActionLabel="New Announcement"
        onEmptyAction={() => navigate('/masjid/admin/announcements/new')}
      >
        <ModernCard>
          <Table>
            <TableHeader>
                  ...
                  <TableCell className="text-right">
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end">
                        <DropdownMenuItem onClick={() => navigate(`/masjid/admin/announcements/edit/${item.id}`)}>
