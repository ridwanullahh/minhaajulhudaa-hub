...
const ManagePrayerTimes = () => {
  ...
  const { data: items, isLoading, error, refetch } = useListData(() => masjidDB.get('prayer_times'));

  const handleDelete = async (id: string) => {
    if (window.confirm('Are you sure you want to delete this record?')) {
      try {
        await masjidDB.delete('prayer_times', id);
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        <div>
          <h1 className="text-2xl font-bold text-foreground">Manage Prayer Times</h1>
          <p className="text-sm text-muted-foreground mt-1">View and manage prayer times</p>
        </div>
        <Link to="/masjid/admin/prayer-times/new">
      ...
      <DataState
        ...
        onEmptyAction={() => navigate('/masjid/admin/prayer-times/new')}
      >
        <ModernCard>
          <Table>
            <TableHeader>
                  ...
                  <TableCell className="text-right">
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end">
                        <DropdownMenuItem onClick={() => navigate(`/masjid/admin/prayer-times/edit/${item.id}`)}>
