...
const ManagePayments = () => {
  ...
  const loadPayments = async () => {
    setIsLoading(true);
    try {
      const data = await schoolDB.get('payments');
      setPayments(data);
    } catch (error) {
  ...
  const handleDelete = async (id: string) => {
    if (window.confirm("Are you sure you want to delete this payment record?")) {
      try {
        ...
        loadPayments(); // Refresh list after deleting
      } catch (error) {
          ...
          <TableBody>
            {payments.map((payment) => (
              <TableRow key={payment.id}>
                ...
                <TableCell>{new Date(payment.enrollmentDate).toLocaleDateString()}</TableCell>
                <TableCell className="text-right">
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end">
                      <DropdownMenuItem onClick={() => navigate(`/school/admin/payments/edit/${payment.id}`)}>
