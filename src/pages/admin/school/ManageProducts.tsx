...
const ManageProducts = () => {
  ...
  const { data: items, isLoading, error, refetch } = useListData(() => schoolDB.get('shop_products'));

  const handleDelete = async (id: string) => {
    if (window.confirm('Are you sure you want to delete this record?')) {
      try {
        await schoolDB.delete('shop_products', id);
    ...
    <div>
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">View and manage products</p>
        </div>
      ...
      <DataState
        ...
        onEmptyAction={() => navigate('/school/admin/products/new')}
      >
        <ModernCard>
          <Table>
            <TableHeader>
                  ...
                  <TableCell className="text-right">
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end">
                        <DropdownMenuItem onClick={() => navigate(`/school/admin/products/edit/${item.id}`)}>
