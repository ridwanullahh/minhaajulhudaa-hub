...
const ManageStaff = () => {
  ...
  const { data: items, isLoading, error, refetch } = useListData(() => schoolDB.get('staff'));

  const handleDelete = async (id: string) => {
    if (window.confirm('Are you sure you want to delete this record?')) {
      try {
        await schoolDB.delete('staff', id);
    ...
    <div>
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">View and manage staff</p>
        </div>
        <Link to="/school/admin/staff/new">
      ...
      <DataState
        ...
        emptyActionLabel="New Staff"
        onEmptyAction={() => navigate('/school/admin/staff/new')}
      >
        <ModernCard>
          <Table>
            <TableHeader>
                  ...
                  <TableCell className="text-right">
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end">
                        <DropdownMenuItem onClick={() => navigate(`/school/admin/staff/edit/${item.id}`)}>
