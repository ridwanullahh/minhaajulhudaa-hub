...
const ManagePrograms = () => {
  ...
  const { data: items, isLoading, error, refetch } = useListData(() => schoolDB.get('programs'));

  const handleDelete = async (id: string) => {
    if (window.confirm('Are you sure you want to delete this record?')) {
      try {
        await schoolDB.delete('programs', id);
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        <div>
          <h1 className="text-2xl font-bold text-foreground">Manage Programs</h1>
          <p className="text-sm text-muted-foreground mt-1">View and manage programs</p>
        </div>
        <Link to="/school/admin/programs/new">
      ...
      <DataState
        ...
        onEmptyAction={() => navigate('/school/admin/programs/new')}
      >
        <ModernCard>
          <Table>
            <TableHeader>
                  ...
                  <TableCell className="text-right">
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end">
                        <DropdownMenuItem onClick={() => navigate(`/school/admin/programs/edit/${item.id}`)}>
