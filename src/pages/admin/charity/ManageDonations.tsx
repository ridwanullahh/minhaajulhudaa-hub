...
const ManageDonations = () => {
  ...
  const { data: items, isLoading, error, refetch } = useListData(() => charityDB.get('donations'));

  const handleDelete = async (id: string) => {
    if (window.confirm('Are you sure you want to delete this record?')) {
      try {
        await charityDB.delete('donations', id);
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        <div>
          <h1 className="text-2xl font-bold text-foreground">Manage Donations</h1>
          <p className="text-sm text-muted-foreground mt-1">View and manage donations</p>
        </div>
        <Link to="/charity/admin/donations/new">
      ...
      <DataState
        ...
        emptyActionLabel="New Donation"
        onEmptyAction={() => navigate('/charity/admin/donations/new')}
      >
        <ModernCard>
          <Table>
            <TableHeader>
                  ...
                  <TableCell className="text-right">
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end">
                        <DropdownMenuItem onClick={() => navigate(`/charity/admin/donations/edit/${item.id}`)}>
