...
const ManageVolunteers = () => {
  ...
  const { data: items, isLoading, error, refetch } = useListData(() => charityDB.get('volunteers'));

  const handleDelete = async (id: string) => {
    if (window.confirm('Are you sure you want to delete this record?')) {
      try {
        await charityDB.delete('volunteers', id);
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        <div>
          <h1 className="text-2xl font-bold text-foreground">Manage Volunteers</h1>
          <p className="text-sm text-muted-foreground mt-1">View and manage volunteers</p>
        </div>
        <Link to="/charity/admin/volunteers/new">
      ...
      <DataState
        ...
        emptyActionLabel="New Volunteer"
        onEmptyAction={() => navigate('/charity/admin/volunteers/new')}
      >
        <ModernCard>
          <Table>
            <TableHeader>
                  ...
                  <TableCell className="text-right">
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end">
                        <DropdownMenuItem onClick={() => navigate(`/charity/admin/volunteers/edit/${item.id}`)}>
