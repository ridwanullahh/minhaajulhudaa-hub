...
const ManageTestimonials = () => {
  ...
  const { data: items, isLoading, error, refetch } = useListData(() => charityDB.get('testimonials'));

  const handleDelete = async (id: string) => {
    if (window.confirm('Are you sure you want to delete this record?')) {
      try {
        await charityDB.delete('testimonials', id);
    ...
    <div>
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">View and manage testimonials</p>
        </div>
        <Link to="/charity/admin/testimonials/new">
      ...
      <DataState
        ...
        onEmptyAction={() => navigate('/charity/admin/testimonials/new')}
      >
        <ModernCard>
          <Table>
            <TableHeader>
                  ...
                  <TableCell className="text-right">
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end">
                        <DropdownMenuItem onClick={() => navigate(`/charity/admin/testimonials/edit/${item.id}`)}>
