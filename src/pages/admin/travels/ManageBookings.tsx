...
const ManageBookings = () => {
  ...
  const { data: items, isLoading, error, refetch } = useListData(() => travelsDB.get('bookings'));

  const handleDelete = async (id: string) => {
    if (window.confirm('Are you sure you want to delete this record?')) {
      try {
        await travelsDB.delete('bookings', id);
    ...
    <div>
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">View and manage bookings</p>
        </div>
        <Link to="/travels/admin/bookings/new">
      ...
      <DataState
        ...
        onEmptyAction={() => navigate('/travels/admin/bookings/new')}
      >
        <ModernCard>
          <Table>
            <TableHeader>
                  ...
                  <TableCell className="text-right">
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end">
                        <DropdownMenuItem onClick={() => navigate(`/travels/admin/bookings/edit/${item.id}`)}>
