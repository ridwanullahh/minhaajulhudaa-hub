      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2">
            ...
            <ModernCard variant="glass" className="p-8">
                ...
                <ModernButton size="lg">Enroll Now (${course.price})</ModernButton>
            </ModernCard>
        </div>
            ...
            <ModernCard className="p-6">
                ...
                <div className="space-y-3">
                    {lessons.map((lesson, index) => (
                        <div key={lesson.id} className="flex items-center p-3 bg-muted rounded-lg hover:bg-primary/5 transition-colors">
                            <PlayCircle className="w-6 h-6 text-primary mr-4"/>
                            <div className="flex-grow">
                                <p className="font-semibold">{index + 1}. {lesson.title}</p>
                                <p className="text-sm text-muted-foreground">{lesson.duration || '5 min'}</p>
                            </div>
                        </div>
                    ))}
                </div>
